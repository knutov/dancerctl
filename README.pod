...
=head3 Usage
  ...
  Where:
  action      - start|stop|restart|status|help|init or startall|stopall|restartall|statusall
                Will start/stop/restart default environment if no 'appname' is specified.
  environment - usually 'development' or 'production' (by default)
...
Default config can be placed in C<recommended:> section to F</etc/dancer.conf> 
otherwise the __DATA__ section of F<dancerctl> script will be used.
...
Not implemented now, but will be in future
  ...
  dancerctl init    [not implemented]
...
Config format: YAML for the F<~/.dancer/$appname> and any ini-style (better 
YAML too) for the F</etc/dancer.conf>
...
It is possible to use to global parameters in %ENV:
  ...
  DANCERCTL_CONF  - default config for dancerctl, instead of /etc/dancer.conf
  DANCERCTL_DEBUG - enable/disable debug mode, has more priority, then in dancer.conf
  
  Examples:
    export DANCERCTL_CONF=/path/to/local/dancer.conf
